...
final class inbound_test extends \advanced_testcase {
    ...
    /**
     * The generated subaddress must use the base64url alphabet so it survives MTAs that
     * reject or rewrite '+', '/' and '=' (e.g. AWS WorkMail). See MDL-71652.
     */
    public function test_generated_subaddress_uses_url_safe_alphabet(): void {
        $handler = $this->helper_create_handler('handler_one');
        $user = $this->getDataGenerator()->create_user();

        $processor = new core_message_inbound_test_helper();
        $processor->set_handler($handler->classname);
        $processor->set_data(42);
        $address = $processor->generate($user->id);

        list($localpart) = explode('@', $address, 2);
        list(, $subaddress) = explode('+', $localpart, 2);

        $this->assertSame(0, preg_match('#[+/=]#', $subaddress),
            'Generated subaddress should be base64url-encoded (no +, / or = characters).');
    }

    /**
     * Addresses generated by older Moodle versions (which used standard base64 with
     * '+', '/' and '=') must still be processed successfully so that messages already
     * sitting in users' inboxes do not silently fail after the upgrade. See MDL-71652.
     */
    public function test_legacy_base64_subaddress_is_still_processed(): void {
        $dataid = 42;
        $handler = $this->helper_create_handler('handler_one');
        $user = $this->getDataGenerator()->create_user();
        ...
        $newaddress = $generator->generate($user->id);

        // Convert the new (base64url, unpadded) address back to the legacy base64 form.
        list($localpart, $domain) = explode('@', $newaddress, 2);
        list($mailbox, $subaddress) = explode('+', $localpart, 2);
        $legacysub = strtr($subaddress, '-_', '+/');
        $legacysub .= str_repeat('=', (4 - strlen($legacysub) % 4) % 4);
        $legacyaddress = $mailbox . '+' . $legacysub . '@' . $domain;

        $parser = new core_message_inbound_test_helper();
        $parser->process($legacyaddress);
        $parsed = $parser->get_data();
        $this->assertEquals($user->id, $parsed->userid);
        $this->assertEquals($dataid, $parsed->datavalue);
        $this->assertEquals($handler->id, $parsed->handlerid);
    }
